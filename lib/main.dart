...
class _ProximoPartidoScreenState extends State<ProximoPartidoScreen> {
  ...
  List<DateTime> _generarSabadosDesde({
    ...
    return List.generate(
      cantidad,
      (i) => inicio.add(Duration(days: i * 7)),
    );
  }
  ...
  List<Map<String, dynamic>> _buildClausuraBase({
    required String categoria,
  }) {
  ...
  List<Map<String, dynamic>> _buildFixtureCompleto({
    ...
    final apertura = _buildAperturaBase(categoria: categoria)
        .map(_convertirAFixturePartido)
        .toList();

    final clausura = _buildClausuraBase(categoria: categoria)
        .map(_convertirAFixturePartido)
        .toList();
  ...
  return fixtureCadetes.where((p) {
    return p['torneo'] == 'Apertura' &&
        p['categoria'] == 'Cadetes' &&
        (p['fechaNumero'] as int) > 5;
  }).map((p) {
    return {
      'rival': p['rival'],
      'fecha': p['fecha'],
  ...
  Navigator.push(
    ...
    MaterialPageRoute(
      builder: (_) => ResumenPartidoFinalizadoScreen(
        partido: partidosFinalizados.first,
      ),
    ),
  );
}
              ...
              child: Column(
                ...
                children: [
                  ...
                  _buildSectionCard(
                    ...
                    child: Column(
                      children: [
                        ...
                        _buildInfoRow('Atajadas', '$_atajadas'),
                        _buildInfoRow('Goles recibidos', '$_golesRecibidos'),
                        _buildInfoRow('Penales en contra', '$_penales'),
                      ],
                    ),
                  ),
                  const SizedBox(height: 16),
                  _buildSectionCard(
                    ...
                    child: Column(
                      children: [
                        _buildInfoRow('Goles a favor', '$_golesAFavor'),
                        _buildInfoRow('Goles en contra', '$_golesEnContra'),
                        ...
                        _buildInfoRow('Pérdidas', '$_perdidas'),
                        _buildInfoRow('Recuperaciones', '$_recuperaciones'),
                      ],
                    ),
                  ),
                  const SizedBox(height: 16),
                  _buildSectionCard(
                    ...
                    child: Column(
                      children: [
                        _buildInfoRow('Exclusiones 2 min', '$_exclusiones2Min'),
                        _buildInfoRow('Tarjetas amarillas', '$_amarillas'),
                        _buildInfoRow('Tarjetas rojas', '$_rojas'),
                      ],
                    ),
                  ),
                  const SizedBox(height: 16),
      ...
      children: [
        Row(
          children: [
            ...
            const SizedBox(width: 10),
            Expanded(
              child: _buildKpiCard('Atajadas', '$_atajadas'),
            ),
          ],
        ),
        const SizedBox(height: 10),
        Row(
          children: [
            Expanded(
              child: _buildKpiCard('Pérdidas', '$_perdidas'),
            ),
            const SizedBox(width: 10),
            Expanded(
...
class FixtureScreen extends StatelessWidget {
  ...
  if (widget.categoria == 'Cadetes' && widget.torneo == 'Apertura') {
    return _buildAperturaBase(categoria: 'Cadetes')
        .map((e) => _convertirAFixturePartido(e))
        .toList();
  }

  if (widget.categoria == 'Cadetes' && widget.torneo == 'Clausura') {
    return _buildClausuraBase(categoria: 'Cadetes')
        .map((e) => _convertirAFixturePartido(e))
        .toList();
  }

  if (widget.categoria == 'Juveniles' && widget.torneo == 'Apertura') {
    return _buildAperturaBase(categoria: 'Juveniles')
        .map((e) => _convertirAFixturePartido(e))
        .toList();
  }

  if (widget.categoria == 'Juveniles' && widget.torneo == 'Clausura') {
    return _buildClausuraBase(categoria: 'Juveniles')
        .map((e) => _convertirAFixturePartido(e))
        .toList();
  }
...
class _PartidoEnJuegoScreenState extends State<PartidoEnJuegoScreen> {
  ...
  if (!_partidoFinalizado) {
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text('El resumen completo se habilita al finalizar el partido'),
      ),
    );
    return;
  }
    ...
    return Container(
      ...
      child: Row(
        children: [
          _buildMiniTeamTag('L'),
          const SizedBox(width: 8),
          _buildMiniShield(assetPath: 'assets/images/san_fernando.png'),
          const Spacer(),
          Column(
            children: [
              Text(
          ...
          const Spacer(),
          _buildMiniShield(assetPath: _rivalShieldAsset()),
          const SizedBox(width: 8),
          _buildMiniTeamTag('V'),
        ],
      ),
